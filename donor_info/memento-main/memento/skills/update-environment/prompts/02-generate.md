...
5. **Write the generated content** to `{{variables.clean_dir}}/{{variables.current_file.target}}` using the Write tool
...
Write the file using the Write tool to `{{variables.clean_dir}}/{{variables.current_file.target}}`. Do NOT write to the target path directly — the workflow engine handles that.

Submit a brief status: "Generated <filename> (N lines)"
